...
export function RevisionTrends({ sceneId, appId }) {
  ...
  useEffect(() => {
    let subscribed = true
    axios
      .get(`/api/scenes/${sceneId}/apps/${appId}/revisions-trends`)
      .then((it) => it.data)
      .then((it) => it.trends)
      .then((it) => {
        if (subscribed) {
  ...
  useEffect(() => {
    ...
    if (trendName) {
      axios
        .get(
          `/api/scenes/${sceneId}/apps/${appId}/revisions-trends/${trendName}`
        )
        .then((it) => it.data)
        .then((it) => it.trends)
        .then((it) => {
  ...
  return (
    <div>
      ...
      {!trendNames && (
        <div>
          <p>
            To visualize revision trends, please use the{' '}
            <strong> gocan create-revision-trends </strong> command first.
          </p>
        </div>
      )}
      {chart}
