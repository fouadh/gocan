...
export function AppDetails() {
  ...
  useEffect(() => {
    let subscribed = true
    axios
      .get(`/api/scenes/${sceneId}/apps/${appId}`)
      .then((it) => it.data)
      .then((it) => {
        if (subscribed) setApplication(it)
      })

    axios
      .get(`/api/scenes/${sceneId}/apps/${appId}/entities`)
      .then((it) => it.data)
      .then((it) => it.entities)
      .then((it) => {
        if (subscribed) setEntities(it)
      })
  ...
  return (
    <>
      <div>
        <h3 className="p-ml-4 ml-4">{application?.name}</h3>
      </div>
