...
export function ComplexityAnalysis({ sceneId, appId }) {
  ...
  useEffect(() => {
    ...
    if (analysis) {
      axios
        .get(
          `/api/scenes/${sceneId}/apps/${appId}/complexity-analyses/${analysis}`
        )
        .then((it) => it.data)
        .then((it) => it.entries)
        .then((it) => {
          if (subscribed) {
            const indentations = it.map((each) => ({
              x: each.date,
              y: each.indentations,
            }))
  ...
  useEffect(() => {
    let subscribed = true
    axios
      .get(`/api/scenes/${sceneId}/apps/${appId}/complexity-analyses`)
      .then((it) => it.data)
      .then((it) => it.analyses)
      .then((it) => {
        if (subscribed) {
        ...
        <div className="chart-options">
          <div className="p-col-12 chart-option">
            <Checkbox
              inputId="cb1"
              value="lines"
              onChange={onComplexityTypeChange}
              checked={complexityTypes.includes('lines')}
            ></Checkbox>
          ...
          <div className="p-col-12 chart-option">
            <Checkbox
              ...
              value="indentations"
              onChange={onComplexityTypeChange}
              checked={complexityTypes.includes('indentations')}
            ></Checkbox>
          ...
          <div className="p-col-12 chart-option">
            <Checkbox
              ...
              value="mean"
              onChange={onComplexityTypeChange}
              checked={complexityTypes.includes('mean')}
            ></Checkbox>
          ...
          <div className="p-col-12 chart-option">
            <Checkbox
              ...
              value="stdev"
              onChange={onComplexityTypeChange}
              checked={complexityTypes.includes('stdev')}
            ></Checkbox>
            <label htmlFor="cb3" className="p-checkbox-label">
              Stdev
            </label>
          ...
          <div className="p-col-12 chart-option">
            <Checkbox
              ...
              value="max"
              onChange={onComplexityTypeChange}
              checked={complexityTypes.includes('max')}
            ></Checkbox>
            <label htmlFor="cb3" className="p-checkbox-label">
              Max
            </label>
  ...
  if (analyses && analyses.length > 0) {
    ...
    analysisSelector = (
      <>
        <p>
          To visualize complexity analyses, please use the{' '}
          <strong> gocan create-complexity-analysis </strong> command first.
        </p>
      </>
    )
  }
